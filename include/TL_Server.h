...
public:
	typedef set<TL_SessionPtr, TL_Session::TL_SessionCmp> SessionType;
	TL_Server();
...
public:
	const TL_SessionPtr & addSession(const TL_SessionPtr& session);
	void removeSession(const TL_SessionPtr& session);
	TL_SessionPtr getSessionPtr(unsigned int session_id);
...
private:
	...
	map<unsigned int, TL_SessionPtr> _id_sessions;
