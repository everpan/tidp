...
bool TL_Mysql::reConnect() {
	...
	if (!mysql_init(&_mysql)) {
		...
		sprintf(errstr, "TL_Mysql Error::connect:%d %s\n", err,
				mysql_error(&_mysql));
		throw TL_Exp(errstr, err);
	}
	MYSQL * _pmysql = mysql_real_connect(&_mysql, _hostname.c_str(),
			_username.c_str(), _password.c_str(), _dbname.c_str(), _port, NULL,
			0);
		...
		if (eno == CR_SERVER_LOST || eno == CR_SERVER_GONE_ERROR) {
			try {
				...
				if (ret != 0) {
					char errstr[200];
					int err = mysql_errno(&_mysql);
					sprintf(errstr, "TL_Mysql Error 001::query:%d %d %s\n", ret,
			...
			char errstr[200];
			sprintf(errstr, "TL_Mysql Error 002::query:%d %d %s\n", ret,
...
TL_Mysql::TL_MysqlRes::~TL_MysqlRes() {
	--_counter;
	//std::cout << "TL_Mysql::TL_MysqlRes::~TL_MysqlRes counter:" << _counter << ":" << (void*) _result << std::endl;
	if (_counter <= 0) {
		destory();
	}
}
...
void TL_Mysql::TL_MysqlRes::destory() {
	if (_result != NULL) {
		//std::cout << "mysql_free_result:" << (void *) _result <<  std::endl;
...
const char* TL_Mysql::TL_MysqlRes::operator[](int fieldno) {
	if (fieldno >= (int) _fields_num) {
		char c[100];
		sprintf(c, "out of rang %d", fieldno);
		throw TL_Exp(c);
	}
	if (_row == NULL) {
		throw TL_Exp("TL_Mysql:TL_MysqlRes:Row is end.");
